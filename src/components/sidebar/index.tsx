                ...
                <div className="relative w-[150px] h-[350px] pointer-events-none">
                    ...
                    <div className="absolute inset-0  z-10 flex flex-col justify-center gap-4 pointer-events-auto">
                        {chest_names.map((item, index) => (
                            <div className="relative group min-[769px]:ml-8 max-w-max" key={index} onMouseEnter={() => { setHovering(true) }}
                                ...
                                <div className="max-[769px]:block hidden ml-2 text-white bg-gray-700 px-2 text-[1.25rem] font-asheader whitespace-nowrap text-center" onClick={() => setHovering(false)}>
                                    {item === "main_page" ? "A Hoy!" : item.replace("_", " ")}
                                </div>
                            </div>
